    ...
    <div class="main">
        <a-config-provider :theme="{
            ...
            <div class="map-panel py-5 px-5">
                <a href="/" class="font-medium px-4">
                    Map Shear
                </a>
                <a class="my-5 bg-[#f9fafb] text-[#1677ff] mr-2 px-2 py-2 text-sm font-medium rounded-md">基本地图配置</a>
                <div>
                    <a>选择地图样式：</a>
                    <a-select ref="select" v-model:value="map_style" style="width: 120px" @change="changeMap(map_style)"
                        :options="styleOptions"></a-select>
                </div>
                <a
                    class="mt-8 mb-5 bg-[#f9fafb] text-[#1677ff] mr-2 px-2 py-2 text-sm font-medium rounded-md">自定义地图配置</a>
                <a>Mapbox Token：</a>
                <a-input v-model:value="user_token" placeholder="Mapbox Token" class="mt-3  max-w-xs" />
                <a class="mt-5">Mapbox 样式链接：</a>
                <a-input v-model:value="user_style" placeholder="mapbox://styles/mapbox/streets-v11"
                    class="mt-3 max-w-xs" />
                <div class="mt-10 flex gap-x-6">
                    <a-button type="primary" @click="customMap">使用自定义地图</a-button>
                    <a-button class="bg-[#3eaf7c] text-white" @click="shearMap">导出地图</a-button>
                </div>
...
const styleOptions = Object.entries(MAP_CONFIG.STYLES).map(([label, value]) => ({ label, value }))
...
const initMap = (token, style) => {
    ...
    let navigationControl = new mapboxgl.NavigationControl({ visualizePitch: true })
